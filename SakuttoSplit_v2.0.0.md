...
## 11. 次バージョン以降の開発ストック（実装しない）

v2.0.0 完了後に、リワードの「見る理由」を足す候補。優先は幹事価値順。詳細仕様は別計画で書く。

| 機能 | リワード | プレミアム（将来 IAP） |
| --- | --- | --- |
| メンバーセット保存 | 1 件保存 | 無制限・起動時復元 |
| 前回会計の復元 | 直近 1 件 | 履歴一覧 |
...
最初の実装コミットはフェーズ 0（契約・資格判定のテストと Config 拡張）とする。画面の見た目はフェーズ 1 から変える。本番ユニット ID の発行はフェーズ 3 と並行して AdMob コンソールで行う（コードに入れるのはフェーズ 5）。
