...
**Store の 2.0.0 は広告（本ファイル）と会計継続をまとめて出す。** 前回復元とメンバーセットの仕様・フェーズは `SakuttoSplit_v2.1.0.md`（文書名は作成時のまま。版番号は 2.0.0）を正とする。広告だけを 2.0.0 として先に提出しない。
...
広告スライス（本ファイル）は完了済み。実装者は `SakuttoSplit_v2.1.0.md` のフェーズ 0 から会計継続を始め、完了後に `MARKETING_VERSION = 2.0.0` で **一度だけ** 提出する。
