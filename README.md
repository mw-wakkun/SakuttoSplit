...
右上 Menu に編成の保存/読込と「今日の広告をオフ」。初期画面の再開カードは 1 タップ。入力中（dirty）の復元だけ左上で、確認 Alert が出る。
...
- **メンバーセット**: グループ編成と端数単位を名前付きで保存する。総額は含めない。無料 1 件。動画を最後まで見ると枠が 1 つ増え、最大 3。適用しても今の総額は変わらない。確認 Alert は出さず、直後の Undo で戻せる。広告オフ用の動画とは報酬が混ざらない。
