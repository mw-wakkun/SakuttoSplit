...
## 12. この次（同一 2.0.0 の第3スライス / 出荷後）

回収チェックリストは `SakuttoSplit_v2.2.0.md` を正とする（2.0.0 に含める）。IAP 以降は 2.0.0 出荷後。

| 順 | 候補 | 理由 |
| --- | --- | --- |
| 1 | IAP 買い切り（広告オフ永続 + セット無制限） | 枠 3 と 24h オフと回収ボードが揃ってから |
| 2 | 端数の四捨五入 / 切り上げ | 計算仕様。テスト先行必須 |
| 3 | シェア画像 | デザイン作業が大きい |
| 4 | OCR / 複数店舗 / ルーレット / 個人名 | 別画面・権限・エンタメ。急がない |

---

## 13. 次のアクション

第2スライスは完了済み。実装者は `SakuttoSplit_v2.2.0.md` のフェーズ 0 から始める。
...
- 2026-09-14: **フェーズ 4 完了**。README に前回復元（無料）とメンバーセット（1 + リワードで最大 3）を追記。`MARKETING_VERSION = 2.0.0`。壊れた session データでも initial で起動するテストを追加。実機の目視は提出前に行う。
